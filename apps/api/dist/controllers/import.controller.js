...
const csvParser_service_1 = require("../services/csvParser.service");
const aiMapper_service_1 = require("../services/aiMapper.service");
...
async function importCsv(req, res) {
    ...
    try {
        ...
        // ── Step 1: Parse CSV ──────────────────────────────────
        const csvResult = (0, csvParser_service_1.parseCsvBuffer)(file.buffer);
        ...
        console.log(`[import] Parsed ${csvResult.totalRows} rows with headers: ${csvResult.headers.join(", ")}`);
        // ── Step 2: AI Mapping ─────────────────────────────────
        const aiResult = await (0, aiMapper_service_1.mapRowsWithAi)(csvResult.rows);
        console.log(`[import] AI mapped ${aiResult.mapped.length} rows, ${aiResult.skipped.length} skipped by AI`);
        // ── Step 3: Validate ───────────────────────────────────
        const validationResult = (0, validator_service_1.validateRecords)(aiResult.mapped, csvResult.rows);
        // Combine AI-skipped + validation-skipped
        const allSkipped = [...aiResult.skipped, ...validationResult.skipped];
        console.log(`[import] Validated: ${validationResult.parsed.length} imported, ${allSkipped.length} total skipped`);
        // ── Step 4: Respond ────────────────────────────────────
        const response = {
            success: true,
            summary: {
