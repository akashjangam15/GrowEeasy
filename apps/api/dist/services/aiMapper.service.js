...
exports.mapRowsWithAi = mapRowsWithAi;
const genai_1 = require("@google/genai");
const openai_1 = __importDefault(require("openai"));
const batchArray_1 = require("../utils/batchArray");
...
function getGroqClient() {
    if (!_groqClient) {
        ...
        if (!apiKey) {
            throw new Error("GROQ_API_KEY is not set. Get a key from https://console.groq.com/keys and add it to your .env file.");
        }
        console.log(`[aiMapper] Using Groq API key: ${apiKey.substring(0, 8)}... model: ${MODEL}`);
        _groqClient = new openai_1.default({
...
async function mapBatchWithGemini(rows) {
    ...
    const response = await ai.models.generateContent({
        model: modelName,
        contents: (0, crmMappingPrompt_1.buildUserPrompt)(rows),
        config: {
            systemInstruction: crmMappingPrompt_1.CRM_MAPPING_SYSTEM_PROMPT,
...
async function mapBatchWithGroq(rows) {
    ...
    const response = await client.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: crmMappingPrompt_1.CRM_MAPPING_SYSTEM_PROMPT,
            },
            {
                role: "user",
                content: (0, crmMappingPrompt_1.buildUserPrompt)(rows),
            },
        ],
        temperature: TEMPERATURE,
...
async function mapBatch(rows) {
    ...
    if (process.env.GEMINI_API_KEY) {
        try {
            ...
            console.warn(`[aiMapper] Gemini mapping failed. Falling back to Groq... Error: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    else {
    ...
    if (filledRecords.length !== rows.length) {
        console.warn(`[aiMapper] Batch size mismatch: sent ${rows.length} rows, got ${filledRecords.length} back`);
    }
    return filledRecords;
}
function isFatalAiError(message) {
    const lowercase = message.toLowerCase();
    return (lowercase.includes("resource_exhausted") ||
 ...
 */
async function mapRowsWithAi(rows) {
    if (rows.length === 0) {
        return { mapped: [], skipped: [] };
    }
    const batches = (0, batchArray_1.batchArray)(rows, BATCH_SIZE);
    const allMapped = [];
    const allSkipped = [];
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        console.log(`[aiMapper] Processing batch ${batchIndex + 1}/${batches.length} (${batch.length} rows)`);
        try {
            const mapped = await (0, retry_1.retry)(() => mapBatch(batch), MAX_RETRIES);
            ...
            console.error(`[aiMapper] Batch ${batchIndex + 1} failed after ${MAX_RETRIES} retries: AI processing failed: ${reason}`);
...
//# sourceMappingURL=aiMapper.service.js.map